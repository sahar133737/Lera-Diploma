...
{
    /// <summary>
    /// Инициализация БД без CreateDatabaseIfNotExists: при изменении модели EF6 иначе падает с
    /// «model has changed» на уже существующей базе. Здесь: Create при отсутствии БД, иначе — только DDL для новых таблиц + сиды.
    /// </summary>
    public sealed class FinancialDbInitializer : IDatabaseInitializer<FinancialDbContext>
    {
        public void InitializeDatabase(FinancialDbContext context)
        {
            if (context == null)
